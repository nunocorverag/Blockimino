...
class Publicacion {
    ...
    public function enviarPublicacion($titulo, $cuerpo, $publicado_para, $nombre_imagen, $tipo_pagina)
    {
       ...
       // + Buscamos todos los hashtags en el cuerpo de la publicación
        preg_match_all('/#\w+/', $cuerpo, $hashtags);
        // + Extraemos los hashtags en un arreglo
        $hashtags = $hashtags[0];

        // Si hay hashtags, los inserta en la tabla de hashtags
        if (!empty($hashtags)) {
            foreach ($hashtags as $hashtag) {
                $query_comprobar_que_hashtag_no_exista = mysqli_query($this->con, "SELECT * FROM hashtags WHERE hashtag='$hashtag'");
                if(!(mysqli_num_rows($query_comprobar_que_hashtag_no_exista) > 0))
                {
                    mysqli_query($this->con, "INSERT INTO hashtags VALUES ('', '$hashtag')");
                }
            }
        }
        ...
        if ($checar_cuerpo_vacio != "" && $checar_titulo_vacio != "")
        {
            ...
            {
                $query_agrega_publicacion = mysqli_query($this->con, "INSERT INTO publicaciones VALUES('', '$titulo', '$cuerpo', '$publicado_por', NULL, '$nombre_imagen', '$fecha_publicado', 'no', '0', '$tipo_pagina')");
    ...
    public function cargarPublicacionesAmigos ($info, $limite){
        ...
        if(mysqli_num_rows($query_info) > 0)
        {
            ...
            while($fila = mysqli_fetch_array($query_info))
            {
                ...
                // + Encontrar hashtags en el cuerpo y convertirlos en enlaces
                $cuerpo = preg_replace('/#(\w+)/', '<a href="publication_hashtag.php?hashtag=$1">#$1</a>', $cuerpo);
    ...
    public function cargarPublicacionesHashtag ($info, $limite){
        ...
        $string_publicacion = "";
        // + Esta query va a obtener todas las publicaciones no borradas y las va a ordenar de forma descendente, es decir, las que se crearon primero, hasta abajo
        $query_info = mysqli_query($this->con, "SELECT * FROM publicaciones WHERE borrado='no' AND cuerpo LIKE '%#$hashtag%'0 ORDER BY id_publicacion DESC");
        ...
        if(mysqli_num_rows($query_info) > 0)
        {
            ...
            while($fila = mysqli_fetch_array($query_info))
            {
                ...
                // + Verificamos si la publicacion pertenece a un grupo y si el usuari pertenece a ese grupo
                $query_verificar_publicacion_grupo = mysqli_query($this->con, "SELECT id_grupo_publicacion FROM publicaciones WHERE id_publicacion='$id_publicacion' AND id_grupo_publicacion IS NOT NULL");
                $pertenece_grupo = true;
                if(mysqli_num_rows($query_verificar_publicacion_grupo) == 1)
                {
                    $fila_id_grupo = mysqli_fetch_array($query_verificar_publicacion_grupo);
                    $id_grupo = $fila_id_grupo['id_grupo_publicacion'];
